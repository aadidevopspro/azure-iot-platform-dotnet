...
namespace Mmm.Iot.Functions.DeploymentSync.Shared.Helpers
{
    public class QueryBuilder
    {
            ...
            string propertyToQuery)
        {
            ...
            {
                ValidateInput(ref identities[i]);
            }

            ValidateInput(ref propertyToQuery);

            var sqlParameterCollection = new SqlParameterCollection();

            var queryBuilder = new StringBuilder("SELECT TOP @top * FROM c WHERE (c[\"CollectionId\"] = @collection");
            sqlParameterCollection.Add(new SqlParameter { Name = "@top", Value = skip + limit });
            sqlParameterCollection.Add(new SqlParameter { Name = "@collection", Value = collection });

            if (identities.Length > 0)
            {
                var parameters = ConvertToSqlParameters("parameterName", identities);
                queryBuilder.Append($" AND c[@devicesProperty] IN ({string.Join(",", parameters.Select(p => p.Name))})");
                sqlParameterCollection.Add(new SqlParameter { Name = "@devicesProperty", Value = propertyToQuery });
        ...
        public static SqlQuerySpec GetDeploymentDeviceModuleDocumentsSqlByKey(
             string key,
             string value)
        {
            var sqlParameterCollection = new SqlParameterCollection();
